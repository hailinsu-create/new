...
**当前版本：0.14.0**（debug APK：`dist/pangchuang-0.14.0-debug.apk`）。`dist/pangchuang-0.13.1-debug.apk`、`dist/pangchuang-0.13.0-debug.apk` 与 `dist/pangchuang-0.12.0-debug.apk` 仍保留。

**定价：演示召唤一直免费（不看真屏）。未解锁时可用自己的 Key 真屏试用 6 次成功陪伴语；一次性 Play 内购（约 $0.99）后不限次数。Debug 包侧载测试时自动解锁，试用计数不适用。**
...
4. 要真屏：在「完整陪伴设置」里填视觉 API（SiliconFlow 等 HTTPS 接口），点 **「完整陪伴」** → 确认录屏说明 → 允许系统「屏幕录制」。未解锁时有 6 次成功真屏试用；用完后需一次性解锁。设置里可随时提前买。
...
| 解锁 | Play 一次性内购 `full_unlock`（约 $0.99）。未买前可用自己的 Key 真屏试用 6 次。试用计数存在本机，和「已解锁」一样可被本地改写。 |
