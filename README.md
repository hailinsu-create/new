...
**Live2D 不成也能用墨汐。** 默认 Cubism WebView；打开 **「墨汐帧动画」** 则用墨汐立绘 PNG（眨眼 + 口型切帧）。Live2D 连续失败也会自动落到墨汐帧。

立绘源与重导出：`python3 scripts/build-moxi-frame-avatars.py`（见 [docs/animation-tech-routes.md](docs/animation-tech-routes.md)）。
...
Live2D 用透明 WebView + Cubism 4（`pixi-live2d-display`）渲染；说话时驱动口型参数，并按文案情绪切换 expression。样例模型来自 [CubismWebSamples](https://github.com/Live2D/CubismWebSamples) 的 Mao，受 Live2D 样例素材许可约束。帧动画为原创 **墨汐** 立绘（`companion_avatar_*.png`，由 `scripts/build-moxi-frame-avatars.py` 生成）。
...
| 演示陪伴语 | 不调 API |
| 使用帧动画 | **墨汐** PNG 表情 + 口型切帧（不加载 Live2D） |
