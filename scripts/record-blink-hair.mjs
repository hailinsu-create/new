#!/usr/bin/env node
/**
 * Focused blink + hair after reel.
 *   npm run dev   # already running
 *   node scripts/record-blink-hair.mjs
 */
...
const PORT = Number(process.env.MOXI_CDP_PORT || 9344);
const BASE = process.env.MOXI_URL || "http://localhost:5173";
const OUT_DIR = process.env.MOXI_OUT || "/opt/cursor/artifacts";
const PROFILE = process.env.MOXI_PROFILE || "/tmp/moxi-blink-hair-profile";
const CHROME = process.env.CHROME_PATH || "google-chrome";
const VIEW_W = 1440;
const VIEW_H = 900;
const OUT = join(OUT_DIR, "moxi-blink-hair-after.mp4");
const RAW = join(OUT_DIR, "moxi-blink-hair-after.raw.mp4");
...
async function findWindow(timeout = 18000) {
  ...
  while (Date.now() - t0 < timeout) {
    try {
      const ids = (await shOut("xdotool search --class MoxiBlinkHair || true")).split(/\s+/).filter(Boolean);
      for (const id of ids) {
...
async function hud(cdp, title, sub) {
  await cdp.evaluate(`(() => {
    ...
    el.innerHTML = "<div style='color:#c9a36a;font-size:10px;letter-spacing:.38em'>墨汐 · after</div><div style='margin-top:4px;font-size:16px'>${title}</div><div style='margin-top:2px;opacity:.8;font-size:12px'>${sub}</div>";
    return true;
  })()`);
}
...
async function runReel(cdp) {
  ...
  await hud(cdp, "闲置眨眼", "随机间歇 · 非对称开合 · 左右眼差");
  await cdp.evaluate(`window.__MOXI.setWind(1)`);
  await cdp.evaluate(`window.__MOXI.setExpression("neutral")`);
  await sleep(7000);

  await hud(cdp, "微风发丝", "刘海 / 侧发 / 流苏不同相位");
  await cdp.evaluate(`window.__MOXI.setWind(0.45)`);
  await cdp.evaluate(`(() => {
    const { fire, r } = window.__gaze;
    const t0 = performance.now();
    const step = () => {
      const t = (performance.now() - t0) / 1000;
      fire(r.left + r.width * (0.5 + 0.18 * Math.sin(t * 0.9)), r.top + r.height * (0.4 + 0.06 * Math.sin(t * 1.1)));
  ...
  await hud(cdp, "强风二次运动", "夸张但不顶死 · 阵风 + 惯性");
  ...
  await cdp.evaluate(`(() => {
    ...
    const step = () => {
      const t = (performance.now() - t0) / 1000;
      fire(r.left + r.width * (0.5 + 0.28 * Math.sin(t * 1.15)), r.top + r.height * (0.42 + 0.08 * Math.cos(t * 0.8)));
  ...
  await hud(cdp, "说话冲量", "口型能量轻推发丝，不甩头");
  await cdp.evaluate(`window.__MOXI.setWind(1)`);
  await cdp.evaluate(`window.__MOXI.speak("月色入庭，风过竹梢。纸鹤还在，我也还在。")`);
  await sleep(6500);

  await hud(cdp, "回到闲置", "眨眼 150/46/208ms · 分层头发");
  await cdp.evaluate(`window.__MOXI.setExpression("neutral")`);
  await cdp.evaluate(`(() => {
    const { fire, r } = window.__gaze;
    fire(r.left + r.width * 0.5, r.top + r.height * 0.42);
