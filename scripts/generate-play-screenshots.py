...
def settings_shot() -> Image.Image:
    ...
    rounded(d, (56, 340, 1024, 620), 36, PANEL)
    d.text((88, 372), "解锁完整陪伴", font=font(34), fill=FOAM[:3])
    d.text((88, 430), "尚未解锁完整陪伴", font=font(26), fill=MIST[:3])
    d.text((88, 478), "一次购买约 $0.99，永久解锁真屏陪伴。", font=font(24), fill=MIST[:3])
    d.text((88, 516), "演示模式始终免费。", font=font(24), fill=MIST[:3])
    rounded(d, (88, 560, 992, 628), 24, AMBER)
    d.text((330, 576), "解锁完整陪伴（$0.99）", font=font(28), fill=INK[:3])

    rounded(d, (56, 660, 1024, 980), 36, PANEL)
    d.text((88, 692), "隐私与合规", font=font(34), fill=FOAM[:3])
    d.text((88, 752), "隐私政策：启动陪伴前需同意", font=font(26), fill=MIST[:3])
    d.text((88, 800), "数据去向：你填写的视觉 API。", font=font(24), fill=MIST[:3])
    d.text((88, 840), "默认模型 Qwen3-VL-8B。HTTPS 传输。", font=font(24), fill=MIST[:3])

    rounded(d, (56, 1040, 1024, 1168), 28, YELLOW)
    d.text((300, 1082), "召唤小旁陪看屏幕", font=font(34), fill=INK[:3])
    rounded(d, (56, 1200, 1024, 1300), 28, (60, 44, 52))
    d.text((320, 1232), "仅演示悬浮窗（不看真屏）", font=font(28), fill=FOAM[:3])
    return img
