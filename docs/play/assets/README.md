...
`screenshot_settings.png` / `screenshot_consent.png` 按文案重建。悬浮窗两张用 `moxi_face.png` 拼出来，给 Console 先垫上「有 Live2D」的图。**提交前请用真机演示模式换掉**（云端模拟器 System UI 会 ANR，截不到真悬浮窗）。真机至少拍：

1. 设置页（含「界面语言」与「解锁完整陪伴」）
