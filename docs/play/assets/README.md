...
`screenshot_settings.png` / `screenshot_consent.png` 按文案重建（陪伴首页，不是调试表单）。悬浮窗两张用 `moxi_face.png`（Live2D 圆窗 bust crop）按约 **96dp** 头像框拼出来，关闭角标在圆外，给 Console 先垫上「有 Live2D」的图。**这些不是真机照片。提交前请用真机演示模式换掉**（云端模拟器 System UI 会 ANR，截不到真悬浮窗）。真机至少拍：
