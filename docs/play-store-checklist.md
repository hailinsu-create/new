...
- [ ] 确认 Cubism SDK / Publication 许可适合你的主体（角色已换成原创墨汐，不再使用虹色 Mao）
- [ ] 申请生产权限并提交审核
...
| 0.11.0 | 悬浮窗角色换成原创国风 Live2D「墨汐」，移除无商用授权的虹色 Mao |
| 0.12.0 | 锁屏真正停截屏；演示不污染完整陪伴；96dp 可贴边悬浮窗；陪伴首页；法律页 Markdown；墨汐图标 |
| 0.13.0 | 同意前不索通知；撤回同意；敏感应用暂停镜像；48dp 关闭与记住位置；电量/540p；Live2D 眨眼与心情；14 语言补译 |
| 0.13.1 | 圆窗拉远（发丝/呼吸可见）；去掉嘴边黑印；嘴型开合加强 |
| 0.14.0 | 真屏试用 6 次（成功 API 气泡）；演示仍免费不看真屏；关闭钮在脸外；发给模型的图遮住小旁；隐私/条款日期 0.14.0；通知改为可选表述 |
| 0.14.1 | 真屏试用从 6 次提到 20 次成功 API 气泡（终身、计数规则不变）；隐私/条款日期 0.14.1 |
