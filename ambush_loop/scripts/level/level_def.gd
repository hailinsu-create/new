...
static func make_yard() -> LevelDef:
	...
	l.title = "第1关 · 院子：交叉封锁"
	l.teaching = "夜巡小队从北门进院子，要在南闸汇合前切断。灰狼补主路巡卫，铁砧宽锥扫东箱侧翼，夜枭长窄锁出口。陷阱：只盯主路，侧翼奔袭会从东廊绕出。青弧朝向才有掩体减免，侧背全伤。失败三种：逃逸、全灭、中止（X 留情报）。"
	l.tutorial = "左卡选 灰狼/铁砧/夜枭（1/2/3）。点掩体部署；青弧=保护方向（来袭减伤 60%，侧背全伤）。黄锥是墙裁切后的真射界。夜枭部署后有淡青观察环（仅准备期）。A/D、右键或底栏↺↻调朝向，空格拉警报锁死。X 中止留情报。逃逸、全灭、中止都穿梭并恢复上轮计划。红线=主路，橙线=侧翼。"
...
static func make_warehouse() -> LevelDef:
	...
	l.title = "第2关 · 仓道：弹药窗口"
	l.teaching = "弹药窗口：仓道里的夜班搬运队会把弹打空。过早开火是陷阱——铁砧最容易空，把弹包给他。东廊橙色油桶在敌人靠近时自动炸（伤及友军），别把灰狼塞进爆心。F 入伏再打，等他们走进黄区。参考：铁砧拿弹包看窗口，夜枭锁闸口。"
	l.tutorial = "G 把唯一弹包交给已部署队员（空弹自动补一次，本轮不能转交；铁砧优先）。F 切「入伏再打」，黄锥变暗，等敌人进黄色伏击区。东廊油桶是预置杀器：敌人踩近才在模拟里炸，执行中不能点爆。红=主路，橙=侧翼。X 中止留情报；时间轴只读。M 静音。"
...
static func make_pump() -> LevelDef:
	...
	l.title = "第3关 · 泵站：关门之后"
	l.teaching = "泵站夜班要过东廊阀门。B 锁门不是稳赢——那是陷阱：侧翼奔袭在决策格改走西侧紫色备用接近，从你没罩住的侧背进来。重布青弧与夜枭锁线；铁砧侧背更危险。开锁两条线都能解，锁门必须改朝向。"
	l.tutorial = "B 切换锁门。锁上后东廊关闭，侧翼在决策格改走作者写好的紫色备用接近，不会自由寻路。青弧没罩住=全伤。夜枭锁出水口。G 弹包一人。红=主路，橙=侧翼（开），紫=关门后的陷阱接近。X 中止留情报；时间轴只读。M 静音。"
	l.escape_cell = Vector2i(31, 19)
	l.cover_defs = [
...
static func make_railcut() -> LevelDef:
	...
	l.teaching = "信号楼要切断两路巡轨：西廊巡卫立刻出发，东廊奔袭晚 3.8 秒才折下来。陷阱是南闸堆人——先到的西廊把弹药打空，延迟东廊再漏。核心墙挡住对射，必须分廊锁线。绊索只能铺一条。"
	l.tutorial = "西廊立刻走脊，东廊从北过道晚 3.8 秒才到。核心设备挡住东西对射。铁砧朝北等东廊，灰狼补西廊，夜枭锁南闸。Tab 绊索一条走廊；G 弹包一人。没有门。红=西廊主路，橙=东廊延迟。X 中止留情报；时间轴只读。M 静音。"
	l.escape_cell = Vector2i(31, 19)
...
static func make_depot() -> LevelDef:
	...
	l.title = "第5关 · 油库：三路合围"
	l.teaching = "油库夜班三路合围：主路巡卫、东廊奔袭、西暗道影探（晚 2.2 秒）。中间油罐挡住对射。陷阱是南闸堆人+忽略西暗道——先到的两路打空弹药，影探再从西夹缝漏。唯一绊索封西暗道（7,11 一带），弹包给铁砧。"
	l.tutorial = "三路合围。西暗道晚 2.2 秒才从西墙夹缝南下。灰狼锁主路，铁砧朝北等东廊，夜枭看南闸；Tab 把唯一绊索铺在西暗道（7,11 一带）。G 弹包一人。没有门、没有油桶。红=主路，橙=东廊，绿=西暗道陷阱。X 中止留情报；时间轴只读。M 静音。"
	l.escape_cell = Vector2i(31, 19)
	l.cover_defs = [
