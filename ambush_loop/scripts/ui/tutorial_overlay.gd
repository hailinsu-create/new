...
const WAREHOUSE_STEPS := [
	...
	{
		"title": "仓道 3 / 3  ·  油桶",
		"body": "东廊橙色油桶是关卡预置杀器：敌人靠近才在模拟结算里引爆（准备期地图上有「爆心」提示），执行中不能点击引爆。别把灰狼/铁砧放进爆破圈。",
	},
]
...
const DEPOT_STEPS := [
	...
	{
		"title": "油库 2 / 2  ·  绊索换人",
		"body": "西暗道影探晚 2.2 秒才从西墙夹缝南下（SETUP 暗芯片倒计时）。灰狼锁主路，铁砧朝北等东廊，夜枭看南闸；把唯一绊索铺在西暗道。先到的两路会把南闸弹药打空。弹包一人。警报后不能微操。",
	},
]
...
const RAILCUT_STEPS := [
	...
	{
		"title": "信号楼 2 / 2  ·  延迟侧翼",
		"body": "东廊奔袭晚 3.8 秒才从北过道折下来（SETUP 图例橙芯片标了延迟）。铁砧朝北锁东廊等他们；灰狼补西廊；夜枭锁南闸防漏。绊索只能铺一条走廊；弹包一人。警报后不能微操。"
	},
]
