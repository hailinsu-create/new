...
const YARD_STEPS := [
	{
		"title": "1 / 3  ·  部署",
		"body": "左侧三张作战卡是灰狼（步枪）、铁砧（机枪）、夜枭（侦察）。点选一张，再点掩体部署。红线主路从北门进南闸，橙线侧翼从东廊绕出——至少一人锁主路、一人锁东廊，只锁一条会漏。",
	},
	{
		"title": "2 / 3  ·  射界与角色",
		"body": "A/D、右键，或底栏 ↺↻ / 拖已部署队员调朝向。黄锥是墙裁切后的真射界；青弧方向来袭才有掩体减免，侧背全伤。灰狼补主路、铁砧宽锥扫东箱侧翼、夜枭长窄锁南闸出口。红=主路，橙=侧翼。",
	},
	{
		"title": "3 / 3  ·  锁死观看",
		"body": "空格或底栏「警报」拉响后计划冻结，不能再微操，只能暂停/变速观看。失败三种：逃逸、全灭、中止（X 留目前情报）。下一世恢复上轮计划，可以改朝向。陷阱路线：橙线侧翼从东廊绕出，只锁红线主路会漏。",
	},
]
...
const WAREHOUSE_STEPS := [
	...
	{
		"title": "仓道 2 / 3  ·  弹包（G）",
		"body": "G 或底栏「弹包」把唯一备用弹包交给已部署队员。空弹时自动补一次本轮起始弹药，不能转交。铁砧宽锥最容易空，优先给他。仓道之后泵站、信号楼、油库也都只有一包。",
	},
	{
		"title": "仓道 3 / 3  ·  油桶",
		"body": "东廊橙色油桶是关卡预置杀器：敌人靠近才在模拟结算里引爆（准备期地图上有「爆心」提示），执行中不能点击引爆。别把灰狼/铁砧放进爆破圈。陷阱路线：橙线侧翼走东廊，过早开火打空后从爆心外漏出。",
	},
]
...
const PUMP_STEPS := [
	...
	{
		"title": "泵站 2 / 2  ·  侧背与出口",
		"body": "紫色备用接近从西侧绕来，青弧没罩住的方向是全伤；铁砧侧背更危险。夜枭适合锁出水口。警报后计划冻结，只能观看。陷阱路线：锁门后橙线改走西侧紫备用接近，不是消失。",
	},
]
...
const DEPOT_STEPS := [
	...
	},
	{
		...
		"body": "西暗道影探晚 2.2 秒才从西墙夹缝南下（SETUP 暗芯片倒计时）。灰狼锁主路，铁砧朝北等东廊，夜枭看南闸；Tab 把唯一绊索铺在西暗道（7,11 一带）。先到的两路会把南闸弹药打空。弹包一人。警报后不能微操。陷阱路线：绿线西暗道晚 2.2 秒，忽略它南闸会被先到两路打空。",
	},
]
...
const RAILCUT_STEPS := [
	...
	{
		"title": "信号楼 2 / 2  ·  延迟侧翼",
		"body": "东廊奔袭晚 3.8 秒才从北过道折下来（SETUP 图例橙芯片标了延迟）。铁砧朝北锁东廊等他们；灰狼补西廊；夜枭锁南闸防漏。绊索只能铺一条走廊；弹包一人。警报后不能微操。陷阱路线：橙线东廊晚 3.8 秒，南闸先被西廊主路打空。"
	},
]
