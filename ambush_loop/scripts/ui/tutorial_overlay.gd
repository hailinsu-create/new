...
const PUMP_STEPS := [
	...
	{
		"title": "泵站 2 / 2  ·  侧背与出口",
		"body": "紫色备用接近从西侧绕来，青弧没罩住的方向是全伤。这关夜枭锁出水口；铁砧把侧背对准紫线——锁门后紫线改道被你罩住，才是这关的爽点。警报后计划冻结，只能观看。陷阱路线：锁门后橙线改走西侧紫备用接近，不是消失。",
	},
]
...
const DEPOT_STEPS := [
	...
	},
	{
		...
		"body": "西暗道影探晚 2.2 秒才从西墙夹缝南下（SETUP 暗芯片倒计时）。灰狼锁主路，铁砧朝北等东廊，夜枭看南闸。绊索就是第四人：Tab 铺在西暗道（7,11 一带），抽中这一下才爽。先到的两路会把南闸弹药打空。弹包一人。警报后不能微操。陷阱路线：绿线西暗道晚 2.2 秒，忽略它南闸会被先到两路打空。",
	},
]
...
const RADIO_STEPS := [
	{
		"title": "电台 1 / 2  ·  灯塔回波",
		"body": "油库切断后，电台还能把下一班叫回来。红线主路立刻走脊，橙线东廊先到一班，绿线西暗道晚 3.6 秒，灯塔回波再在 5.2 秒把增援送上东廊。中间碟厅挡住对射。",
	},
	{
		"title": "电台 2 / 2  ·  绊索与回波",
		"body": "铁砧朝北等 5.2 秒灯塔回波——等住这一枪才爽。灰狼锁主路，夜枭看南闸。绊索就是第四人：Tab 铺在西暗道（7,11 一带）。弹包一人。警报后不能微操。陷阱路线：绿线西暗道 3.6 秒、橙线回波 5.2 秒，南闸堆人会先被打空。",
	},
]
...
const RAILCUT_STEPS := [
	...
	{
		"title": "信号楼 2 / 2  ·  延迟侧翼",
		"body": "东廊奔袭晚 3.8 秒才从北过道折下来（SETUP 图例橙芯片标了延迟）。这关必须带铁砧朝北等东廊——等住这一枪才爽。灰狼补西廊；夜枭锁南闸防漏。绊索只能铺一条走廊；弹包一人。警报后不能微操。陷阱路线：橙线东廊晚 3.8 秒，南闸先被西廊主路打空。"
	},
]
