...
## Window dump for Ambush Loop v0.3.0 Commandos/WW2 reeval.
## Walks title stamp → SCOUT knives → crate search → Kar98k stamp →
## alarm gate → knife leak → ref ALERT → SWEEP → extract, then six-night
## empty SCOUT + named crates + touch HUD. Script-driven, not a human mouse.

const SAVE_PATH := "user://ambush_loop.cfg"
const SETTINGS_PATH := "user://ambush_loop_settings.cfg"
const OUT := "res://docs/eval_v0.3.0"
...
func _run() -> void:
	...
	await _settle(4)
	main.raid_advance_search(0.18)
	await _settle(6)
	_hud(main, "yard_crate_search")
	print("DUMP_SEARCH weapon=", main.operators[0].weapon_id, " searching=", main.operators[0].is_searching() if main.operators[0].has_method("is_searching") else "?")
	await _save("10_scout_crate_search")
	main.raid_advance_search(0.5)
	await _settle(8)
	_guns(main, "yard_after_rifle")
	_hud(main, "yard_after_rifle")
	await _save("11_scout_yard_rifle")

	# Named WWII model on the card (catalog apply — perceivable stamp).
	main.operators[0].apply_weapon("kar98k", true)
	main._update_hud()
	if main.has_method("_update_role_cards"):
		main._update_role_cards()
	await _settle(8)
	_guns(main, "yard_kar98k")
	await _save("12_scout_yard_kar98k")

	# Named crate on the floor if it spawned (m1911 @ 20,16).
	main.operators[0].stop_move()
	main.operators[0].global_position = main.grid.cell_to_world_center(Vector2i(20, 16))
	main._select_op(0)
	await _settle(8)
	_hud(main, "yard_m1911_cell")
	await _save("13_scout_yard_m1911_cell")
