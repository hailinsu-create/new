...
## Window dump for Ambush Loop v0.3.1 Commandos/WW2 reeval.
## Walks title stamp → SCOUT knives → crate search (Kar98k from yard rifle crate) →
## alarm gate → knife leak (先搜枪) → ref ALERT cinema 警报中 → SWEEP → extract,
## then six-night empty SCOUT + named crates + touch HUD 需枪. Script-driven.

const SAVE_PATH := "user://ambush_loop.cfg"
const SETTINGS_PATH := "user://ambush_loop_settings.cfg"
const OUT := "res://docs/eval_v0.3.1"
...
func _run() -> void:
	...
	# MG34 class-resolved crate (26,12).
	main.operators[0].stop_move()
	main.operators[0].global_position = main.grid.cell_to_world_center(Vector2i(26, 12))
	main._select_op(0)
	await _settle(8)
	_hud(main, "yard_mg34_cell")
	await _save("12_scout_yard_mg34")

	# Named crate on the floor (m1911 @ 21,17 — walkable).
	main.operators[0].stop_move()
