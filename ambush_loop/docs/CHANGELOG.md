...
Release tag: `v0.5.38-touch-slide`

Same RAID as 0.5.37 (`SCOUT → ALERT → SWEEP`, auto fire + auto nade). Phone default stays 简配夜袭.

### Play (phone)
- **西巷跟镜头**: stop using world zoom as the follow camera. West alley stays ~1.0 (floor ≥0.85). The camera pans/crops so the trio + yellow cone stay on screen; the courtyard is not a postage stamp. Dest cells stay lead (7,12), first (6,6) six along-file (dx=1, not hugging the west wall), second (5,16) two back / four the other way. Span 6. West-follow dest world still clamps onto walkable floor (detour ≤6).
- **人影 / 观察环**: cluster body scale and observation-ring scale restored to 1.0 so soldiers are readable, not noise. Rings keep the courtyard / along-file offset. Yellow-cone fade kept. Gameplay cone / ambush / RAID rules unchanged.
- **绕箱南走廊**: unchanged from 0.5.26–0.5.37. Full path y14=0 (mid x≈21 included), east overshoot 0, axis_run 0. Far-side bow sits on the y=13 rim (~15px; far-cap constant still 20).
- Forced-touch path covers settle-on-ring, bidirectional ↺/↻, west trio + faded cone + dest (6,6)/(5,16) + detour ≤6 + readable bodies/rings + pan/crop follow at ~1.0, compact narrow pack, crate-face axis_run 0, and south-corridor **full path** y≤13. 0538 pack is slim so the suite finishes without a 10-minute cut.

### Android
- `versionName` 0.5.38 / `versionCode` 44 (`com.ambushloop.game`).

### How to tell you have this build
1. Title **v0.5.38 COMMANDOS/WW2**.
2. Compact bar still stacks **匍匐** over **背包**; **↺ / ↻** sit next to **需枪**. Hold or swipe either way.
3. West alley **跟**: dests (7,12)/(6,6)/(5,16), span 6. On-ring extra ~44 side / ~8–12 back. Bodies and observation rings at ~1.0. Follow camera at ~1.0 (pan/crop). Yellow cone still faded. South-corridor crate arc still y≤13 at x≈21.

## 0.5.37 — Phone feel: #1 dest (6,6) along-file + west camera 0.40

Sideload APK: `dist/AmbushLoop-v0.5.37-touch-slide.apk`  
Release tag: `v0.5.37-touch-slide`

Same RAID as 0.5.36 (`SCOUT → ALERT → SWEEP`, auto fire + auto nade). Phone default stays 简配夜袭.

### Play (phone)
- **西巷落点**: first follower pulled one more along-file cell. Lead (7,12), first (6,6) six along-file (dx=1, not hugging the west wall), second stays (5,16) two back / four the other way — not pushed further toward the south wall. Span 6. West-follow dest world still clamps onto walkable floor (detour ≤6). Camera raised to ~0.40 (leftover cap already ~0.44) so the still-sticky lead→#2 world gap (~190.9) reads on screen. Bodies and observation rings stay on the visual floor (~0.16). Rings keep the courtyard / along-file offset. Yellow-cone fade kept. Gameplay cone / ambush / RAID rules unchanged.
- **绕箱南走廊**: unchanged from 0.5.26–0.5.36. Full path y14=0 (mid x≈21 included), east overshoot 0, axis_run 0. Far-side bow sits on the y=13 rim (~15px; far-cap constant still 20).
- Forced-touch path covers settle-on-ring, bidirectional ↺/↻, west trio + faded cone + dest (6,6)/(5,16) + detour ≤6 + floor bodies/rings + ring extra cut + camera 0.40, compact narrow pack, crate-face axis_run 0, and south-corridor **full path** y≤13. 0537 pack is slim so the suite finishes without a 10-minute cut.

### Android
- `versionName` 0.5.37 / `versionCode` 43 (`com.ambushloop.game`).

### How to tell you have this build
1. Title **v0.5.37 COMMANDOS/WW2**.
2. Compact bar still stacks **匍匐** over **背包**; **↺ / ↻** sit next to **需枪**. Hold or swipe either way.
3. West alley **跟**: dests (7,12)/(6,6)/(5,16), span 6. On-ring extra ~44 side / ~8–12 back. Bodies and observation rings at the ~0.16 visual floor. Camera at ~0.40. Yellow cone still faded. South-corridor crate arc still y≤13 at x≈21.
...
Release tag: `v0.5.35-touch-slide`

Same RAID as 0.5.34 (`SCOUT → ALERT → SWEEP`, auto fire + auto nade). Phone default stays 简配夜袭.

### Play (phone)
- **西巷落点**: dest cells spread the shortest lead→#2 edge. Lead (7,12), first (6,7) five along-file (dx=1, not hugging the west wall), second (5,16) two back / four the other way. Span 5. West-follow dest world clamps onto walkable floor so a 4-south stand no longer wraps a 22-cell courtyard detour (detour ≤6). Camera raised to ~0.32 so the larger lead→#2 world gap (~190.9) reads on screen. Bodies and observation rings stay on the visual floor (~0.16). Rings keep the courtyard / along-file offset. Yellow-cone fade kept. Gameplay cone / ambush / RAID rules unchanged.
- **绕箱南走廊**: unchanged from 0.5.26–0.5.34. Full path y14=0 (mid x≈21 included), east overshoot 0, axis_run 0. Far-side bow sits on the y=13 rim (~15px; far-cap constant still 20).
- Forced-touch path covers settle-on-ring, bidirectional ↺/↻, west trio + faded cone + dest (6,7)/(5,16) + detour ≤6 + floor bodies/rings + ring extra cut + camera 0.32, compact narrow pack, crate-face axis_run 0, and south-corridor **full path** y≤13. 0535 pack is slim so the suite finishes without a 10-minute cut.

### Android
- `versionName` 0.5.35 / `versionCode` 41 (`com.ambushloop.game`).

### How to tell you have this build
1. Title **v0.5.35 COMMANDOS/WW2**.
2. Compact bar still stacks **匍匐** over **背包**; **↺ / ↻** sit next to **需枪**. Hold or swipe either way.
3. West alley **跟**: dests (7,12)/(6,7)/(5,16), span 5. On-ring extra ~44 side / ~8–12 back. Bodies and observation rings at the ~0.16 visual floor. Camera at ~0.32. Yellow cone still faded. South-corridor crate arc still y≤13 at x≈21.

## 0.5.34 — Phone feel: west ring extra cut + camera 0.30

Sideload APK: `dist/AmbushLoop-v0.5.34-touch-slide.apk`  
Release tag: `v0.5.34-touch-slide`

Same RAID as 0.5.33 (`SCOUT → ALERT → SWEEP`, auto fire + auto nade). Phone default stays 简配夜袭.

### Play (phone)
- **西巷落点**: on-ring extra cut (~44px side / ~8–12px back; was ~60 + 12–28 west) so #2's ring no longer punches the west wall. Dest cells stay lead (7,12), first (6,7) five along-file (dx=1), second (5,15) two back / three the other way. Span 5. A four-south dest still wraps a 22-cell detour, so #2 is not pushed further this round. Camera raised to ~0.30 so the lead→#2 world gap (~184.6) reads larger on screen. Bodies and observation rings stay on the visual floor (~0.16). Rings keep the courtyard / along-file offset. Yellow-cone fade kept. Gameplay cone / ambush / RAID rules unchanged.
- **绕箱南走廊**: unchanged from 0.5.26–0.5.33. Full path y14=0 (mid x≈21 included), east overshoot 0, axis_run 0. Far-side bow sits on the y=13 rim (~15px; far-cap constant still 20).
- Forced-touch path covers settle-on-ring, bidirectional ↺/↻, west trio + faded cone + dest (6,7)/(5,15) + floor bodies/rings + ring extra cut + camera 0.30, compact narrow pack, crate-face axis_run 0, and south-corridor **full path** y≤13. 0534 pack is slim so the suite finishes without a 10-minute cut.

### Android
- `versionName` 0.5.34 / `versionCode` 40 (`com.ambushloop.game`).

### How to tell you have this build
1. Title **v0.5.34 COMMANDOS/WW2**.
2. Compact bar still stacks **匍匐** over **背包**; **↺ / ↻** sit next to **需枪**. Hold or swipe either way.
3. West alley **跟**: dests (7,12)/(6,7)/(5,15), span 5. On-ring extra ~44 side / ~8–12 back. Bodies and observation rings at the ~0.16 visual floor. Camera at ~0.30. Yellow cone still faded. South-corridor crate arc still y≤13 at x≈21.

## 0.5.33 — Phone feel: west dests spread (span 5)

Sideload APK: `dist/AmbushLoop-v0.5.33-touch-slide.apk`  
Release tag: `v0.5.33-touch-slide`

Same RAID as 0.5.32 (`SCOUT → ALERT → SWEEP`, auto fire + auto nade). Phone default stays 简配夜袭.

### Play (phone)
- **西巷落点**: dest cells spread further to reduce mash. Lead (7,12), first follower (6,7) five along-file (dx=1, not hugging the west wall), second (5,15) two back / three the other way. Span 5. Gameplay cone / ambush / RAID rules unchanged. Bodies and observation rings stay on the visual floor (~0.16). Rings keep the courtyard / along-file offset. Camera stays at the ~0.26 clamp so the larger file still fits. Yellow-cone fade kept.
- **绕箱南走廊**: unchanged from 0.5.26–0.5.32. Full path y14=0 (mid x≈21 included), east overshoot 0, axis_run 0. Far-side bow sits on the y=13 rim (~15px; far-cap constant still 20).
- Forced-touch path covers settle-on-ring, bidirectional ↺/↻, west trio + faded cone + dest span 5 + floor bodies/rings + ring courtyard offset + camera 0.26, compact narrow pack, crate-face axis_run 0, and south-corridor **full path** y≤13. 0533 pack is slim so the suite finishes without a 10-minute cut.

### Android
- `versionName` 0.5.33 / `versionCode` 39 (`com.ambushloop.game`).

### How to tell you have this build
1. Title **v0.5.33 COMMANDOS/WW2**.
2. Compact bar still stacks **匍匐** over **背包**; **↺ / ↻** sit next to **需枪**. Hold or swipe either way.
3. West alley **跟**: dests (7,12)/(6,7)/(5,15), span 5. Bodies and observation rings at the ~0.16 visual floor. Camera at the ~0.26 clamp. Yellow cone still faded. South-corridor crate arc still y≤13 at x≈21.

## 0.5.32 — Phone feel: west dests spread (span 4)

Sideload APK: `dist/AmbushLoop-v0.5.32-touch-slide.apk`  
Release tag: `v0.5.32-touch-slide`

Same RAID as 0.5.31 (`SCOUT → ALERT → SWEEP`, auto fire + auto nade). Phone default stays 简配夜袭.

### Play (phone)
- **西巷落点**: dest cells spread to reduce mash. Lead (7,12), first follower (6,8) four along-file (dx=1, not hugging the west wall), second (5,15) two back / three the other way. Span 4. Gameplay cone / ambush / RAID rules unchanged. Bodies and observation rings stay on the visual floor (~0.16). Rings keep the courtyard / along-file offset. Camera stays at the ~0.26 clamp. Yellow-cone fade kept.
- **绕箱南走廊**: unchanged from 0.5.26–0.5.31. Full path y14=0 (mid x≈21 included), east overshoot 0, axis_run 0. Far-side bow sits on the y=13 rim (~15px; far-cap constant still 20).
- Forced-touch path covers settle-on-ring, bidirectional ↺/↻, west trio + faded cone + dest span 4 + floor bodies/rings + ring courtyard offset + camera 0.26, compact narrow pack, crate-face axis_run 0, and south-corridor **full path** y≤13. 0532 pack is slim so the suite finishes without a 10-minute cut.

### Android
- `versionName` 0.5.32 / `versionCode` 38 (`com.ambushloop.game`).

### How to tell you have this build
1. Title **v0.5.32 COMMANDOS/WW2**.
2. Compact bar still stacks **匍匐** over **背包**; **↺ / ↻** sit next to **需枪**. Hold or swipe either way.
3. West alley **跟**: dests (7,12)/(6,8)/(5,15), span 4. Bodies and observation rings at the ~0.16 visual floor. Camera at the ~0.26 clamp. Yellow cone still faded. South-corridor crate arc still y≤13 at x≈21.

## 0.5.31 — Phone feel: west bodies/rings at visual floor + camera clamp

Sideload APK: `dist/AmbushLoop-v0.5.31-touch-slide.apk`  
Release tag: `v0.5.31-touch-slide`

Same RAID as 0.5.30 (`SCOUT → ALERT → SWEEP`, auto fire + auto nade). Phone default stays 简配夜袭.

### Play (phone)
- **西巷人影 / 观察环 / 镜头**: dest cells kept from 0.5.25–0.5.30 — lead (7,12), first follower (6,9) three along-file (dx=1, not hugging the west wall), second (5,14). Span 3. No dest-cell stretch. Bodies and observation rings sit on the visual floor (~0.16). Rings offset one more along-file / courtyard step (~76px radial + side stagger, +46px east) so the trio + yellow cone still read as three bodies on the same span-3 cells. Camera pulls to the ~0.26 clamp. Yellow-cone fade kept.
- **绕箱南走廊**: unchanged from 0.5.26–0.5.30. Full path y14=0 (mid x≈21 included), east overshoot 0, axis_run 0. Far-side bow sits on the y=13 rim (~15px; far-cap constant still 20).
- Forced-touch path covers settle-on-ring, bidirectional ↺/↻, west trio + faded cone + dest span 3 + floor bodies/rings + ring courtyard offset + camera 0.26, compact narrow pack, crate-face axis_run 0, and south-corridor **full path** y≤13. 0531 pack is slim so the suite finishes without a 10-minute cut.

### Android
- `versionName` 0.5.31 / `versionCode` 37 (`com.ambushloop.game`).

### How to tell you have this build
1. Title **v0.5.31 COMMANDOS/WW2**.
2. Compact bar still stacks **匍匐** over **背包**; **↺ / ↻** sit next to **需枪**. Hold or swipe either way.
3. West alley **跟**: dests still (7,12)/(6,9)/(5,14), span 3. Bodies and observation rings at the ~0.16 visual floor. Camera at the ~0.26 clamp. Yellow cone still faded. South-corridor crate arc still y≤13 at x≈21.
