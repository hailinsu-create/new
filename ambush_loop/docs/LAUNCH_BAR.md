...
| Not 8+ | No FOW, no human balance record, still no sprite pipeline or VO. Five authored yards are in (`railcut` + `depot`); 8+ still wants playtest. No sixth dock yard. |
...
| A5 | 开始行动 → **mission list** (5 levels, lock / 可出击 / 首通 from `ambush_loop.cfg`) → briefing → `main.tscn`. Sequential unlock; cannot skip locked | Done |
...
| C9 | First visit per level: yard 3-step; warehouse F/G/barrel; pump B/flank; railcut dual-corridor delay; depot three-route + 2.2s west sneak + tripwire. Page 1 cannot be skipped via dimmer; auto-mark seen on dismiss | Done |
...
| Sixth authored mission / FOW / sprites / Play AAB | Five yards ship: yard / warehouse / pump / railcut / **depot** (油库：三路合围 + 西暗道 2.2s + 绊索). Stage D still deferred |
...
Content note: mission list is **5** authored yards. `railcut` unlocks after pump clear; `depot` (油库：三路合围) unlocks after railcut. Smoke runs documented reference wins (railcut slots 1,4,5 facings 270/270/180; depot 1,4,5 facings 270/270/180 + tripwire at `(7,11)`), then campaign credits must name 油库.
