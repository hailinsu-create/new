...
# 按版本命名的朋友包（可进仓）：
# cp build/android/AmbushLoop-debug.apk dist/AmbushLoop-v0.5.16-touch-slide.apk
```

当前试玩包：`dist/AmbushLoop-v0.5.16-touch-slide.apk`（`versionName` 0.5.16 / `versionCode` 22）。不要装 GitHub `ambush-loop-playtest-0.2.0` 的同名 `AmbushLoop-playtest.apk`。
...
4. 标题应显示 **v0.5.16 COMMANDOS/WW2**。院子里点底栏肖像选人，点地走路（短拖拖图、长按奔跑），靠近匣/岗哨出热区（贴身/侧面绕背会绕岗身画完整虚线，至少 4 格到背后，不是 2 格残段）。未选中肖像点右上角 **跟** 跟上（领队边走跟班连续走；跟上格偏侧后会收到领队正后方，多人左右错开仍偏后；西巷跟上停在正后方左右错开一格对角，不贴西墙叠成一团，三人观察环收小少叠黄锥，镜头在三人+黄锥聚在一起时拉到约 0.54 并保持，落点再错开半格、人影略缩小；领队只拧射界时跟上格随连续朝向换格，点一次或按住 ↻（15°）落点就绕槽位环走，不等到约 90°，圆弧点钳在可行走格上不穿箱；停住不拧则落点再稳一轮；停在黄锥外、不贴锥沿；锥东领队时西巷跟班短绕 2–3 格，不傻等）。点肖像身体/名字/血条/枪印/编号选人，点角标才跟上（角标略抬高，不吃枪印和编号）。脚下无「射界」小字。点队友身体不会抢走选中。匍匐躲黄锥，点 **需枪** 拉警报（警报中自动丢雷）。匣上枪名默认隐藏。
