...
# 按版本命名的朋友包（可进仓）：
# cp build/android/AmbushLoop-debug.apk dist/AmbushLoop-v0.5.3-touch-smooth.apk
```

当前试玩包：`dist/AmbushLoop-v0.5.3-touch-smooth.apk`（`versionName` 0.5.3 / `versionCode` 9）。不要装 GitHub `ambush-loop-playtest-0.2.0` 的同名 `AmbushLoop-playtest.apk`。
...
4. 标题应显示 **v0.5.3 COMMANDOS/WW2**。院子里点底栏肖像选人，点地走路（短拖拖图、长按奔跑），靠近匣/岗哨出热区（背面出绕背，路径绕开黄锥），未选中肖像点 **跟** 跟上，匍匐躲黄锥，点 **需枪** 拉警报（警报中自动丢雷）。北墙不再堆敌1/主路字。
