...
# 按版本命名的朋友包（可进仓）：
# cp build/android/AmbushLoop-debug.apk dist/AmbushLoop-v0.6.3-alert-chrome.apk
```

当前试玩包：`dist/AmbushLoop-v0.6.3-alert-chrome.apk`（`versionName` 0.6.3 / `versionCode` 52）。不要装 GitHub `ambush-loop-playtest-0.2.0` 的同名 `AmbushLoop-playtest.apk`。
