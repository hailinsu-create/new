...
name: grok
description: >-
  Spend the grok.com CLI account quota from Cursor. Use when the user types /grok,
  用 grok, 用 grok 额度, 走 grok.com, 喊 grok, grok cli, 用另一个 grok, or asks to run the
  task on the grok.com login instead of Cursor-billed Grok. Do not use for ordinary
  Cursor Grok work unless one of those triggers is present.
icon: terminal
color: cyan
metadata:
  surfaces:
...
1. Run the doctor from the workspace root:
   ...
   Do not continue until `status` shows a grok.com session.

2. Write the user's task (plus only the repo context they already named) to a prompt file. Do not solve it yourself first.

3. Run Grok CLI. Give the shell a long budget (several minutes); this is a full agent turn on grok.com:

   ```bash
   bash .cursor/skills/grok/scripts/run.sh run --file /tmp/grok-prompt.txt
   ```

   Optional: `GROK_CLI_MODEL=grok-4.6` or `GROK_CLI_CWD` if you must override defaults. Default cwd is the workspace.

4. Return Grok's stdout to the user. Prefix one short line that the work ran on **grok.com CLI quota**, not Cursor Grok. If Grok edited files, summarize those paths from the output or `git status`; do not redo the edits.
