#!/usr/bin/env bash
# WORK runner: OpenCode Go → DeepSeek V4.1 Flash (ds 4.1) at --variant max.
# No --auto (slow per user). Override: OPENCODE_DS41_VARIANT=high|max|...
set -euo pipefail
export PATH="${HOME}/.opencode/bin:${PATH}"
MODEL="${OPENCODE_DS41_MODEL:-opencode-go/deepseek-v4.1-flash}"
VARIANT="${OPENCODE_DS41_VARIANT:-max}"
...
exec opencode run -m "$MODEL" --variant "$VARIANT" "$@"
